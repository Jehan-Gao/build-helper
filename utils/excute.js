...
const { YARN_INSTALL, GIT_ADD, GIT_COMMIT, GIT_PUSH, GIT_CHECKOUT, GIT_PULL } = require('./command')
...
async function excuteNpmPkgChange(data) {
  ...
  try {
    ...
    if (!pkgJSON) throw `no such file -> package.json`
    if (pkgJSON) {
      const pkgJSONObj = JSON.parse(pkgJSON)
      let index = 0
      const isAllExist = pkgInfoList.some((pkgInfo, index) => {
        if (!(pkgInfo.pkgName in (pkgJSONObj.devDependencies) && pkgJSONObj.devDependencies)) {
          index = index
          return false
        }
        return true
      ...
      if (!isAllExist) {
        throw `Not found '${pkgInfoList[index].pkgName}' in package.json`
      }
...
function excuteInstall(data) {
  const { dirPath } = data
  return new Promise((resolve, reject) => {
    exec(`cd ${dirPath} && ${YARN_INSTALL}`, (err, stdout, stderr) => {
      if (err) reject(err)
...
function excutePushOrigin(data) {
  ...
  const changeModuleMsg = pkgInfoList.map(pkgInfo => {
  ...
  const path = resolvePath(`${dirPath}`)
  return new Promise((resolve, reject) => {
    exec(`cd ${path} && ${GIT_ADD} && ${GIT_COMMIT(commitMsg)} && ${GIT_PUSH(branchName)}`, (err, stdout, stderr) => {
      // if (err) reject(err)
      // console.log(err, 'err')
      // reject(stderr)
